# GEMINI - Portal de Aplicaciones

Portal modular Full Stack con backend Node.js/Express y frontend Vue 3 con arquitectura de componentes extensible inspirada en Directus.
...
- Axios configurado
- Layout principal
- Componentes UI base iniciales (VButton, VInput)
- Store de ejemplo (counter)
- Composables `useApi.ts` y `useCollection.ts`
- Sistema de Autenticación (Store, Vista de Login, Router Guards)
- Componentes UI base adicionales (`VSelect`, `VCard`, `VSpinner`, `VCheckbox`, `VTextarea`, `VDialog`, `VTable`, `VBadge`, `VIcon`)
- Sistema de Registro de Componentes (registry.ts, components.ts, main.ts actualizado)
- Interfaces Modulares: `TextInput`, `NumberInput`, `DatePicker`, `FileUpload`
- Displays Modulares: `Text`, `Badge`, `Date`, `Avatar`
- Layouts Modulares: `TableLayout`, `GridLayout`
- Panels Modulares: `MetricPanel`
- Integración con API
...
#### 1.1 Completar Composables Base ✅
```
src/composables/
├── useApi.ts          # Cliente HTTP con interceptores
└── useCollection.ts   # CRUD operations wrapper
```

#### 1.2 Completar Componentes UI Base ✅
```
src/components/ui/
├── VButton.vue        ✅
├── VInput.vue         ✅
...
src/router/
└── index.ts          # Router guards actualizados
```

### Fase 2: Sistema de Componentes Modulares ✅

#### 2.1 Interfaces (Form Inputs)
- TextInput ✅
- NumberInput
- DatePicker
- FileUpload

#### 2.2 Displays (Data Visualization)
- Text
- Badge
- Date
- Avatar

#### 2.3 Layouts (Page Structures)
- TableLayout ✅
- GridLayout

#### 2.4 Panels (Dashboard Widgets)
- MetricPanel

### Fase 3: Sistema de Registro ✅
```
src/components/
└── registry.ts       # Sistema de auto-registro

src/stores/
└── components.ts     # Store de componentes registrados
```
...
<template>
  ...
  >
    ...
    <template #rowActions="{ item }">
      ...
      <button @click="handleDelete(item)">Eliminar</button>
    </template>
  </TableLayout>
</template>
...
import VButton from '@/components/ui/VButton.vue'; // Assuming VButton is imported

const fields = [
  { key: 'name', label: 'Nombre', display: 'text' },
  { key: 'status', label: 'Estado', display: 'badge' },
  { key: 'createdAt', label: 'Creado', display: 'date' },
];

const { items, loading, fetch } = useCollection('/users');
...
const handleSelectionChange = (selected: Set<any>) => {
...
# Utilidades
npm run lint            # Linter
npm run lint:fix        # Linter con autocorrección
